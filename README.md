...
| 6 | **Shallow review trap** — the same LLM that wrote the code also reviews it. Green tests are not evidence of correctness when the same AI wrote both. | Ox Security "Army of Juniors" 2025 (AI-generated code deployed to production with critical flaws) | Verification uses `context: "fresh"` subagent reviewers (clean context, same model). Verification now also documents cross-model review in `references/cli-tools/cross-model-review.md` — commands for `pi -p`, `opencode --file`, `claude -p`, `codex` to invoke a different CLI/agent externally. Mutation testing is recommended. Execution Critique includes "Don't trust the same model" warning. | **Opt-in, not automatic.** Cross-model review is documented and available, but the workflow does NOT auto-detect which CLIs are installed or auto-invoke them. It requires an explicit subagent step. If only pi.dev is available, `pi -p` runs the same model in a separate process — better than degraded context, but not a different model. True cross-model independence requires at least two distinct provider CLIs on the system. |
...
| 8 | **Plan staleness** — plans are generated against one snapshot of the codebase; by the time execution starts, the target has changed | Superpowers Issue #989 (open); inherent to any multi-stage pipeline | Each stage re-reads artifacts from disk. Execution has a Context Rot Check that re-reads spec-tech.md from disk (not memory). Gate stage freezes spec-product.md after Plannotator approval (no further edits without re-submission). Git worktree check before execution creates isolated branches. | **No automatic staleness detection.** Re-reading from disk ensures you work with the latest plan version, but the LLM does NOT automatically compare the plan against current target files — it must be explicitly instructed, and the workflow provides no automated mechanism for this. In practice, staleness is caught only when a file doesn't exist or the diff is obviously incompatible. Subtle staleness (renamed functions, refactored modules) passes through undetected. |
| 9 | **Pipeline memory loss** — the pipeline has no cross-session memory of its own failure patterns. The same mistake repeats in every cycle. | Flamehaven 2026 (AI pipeline analysis); validated by CODESKILL (arXiv 2026), FORGE (arXiv 2026), Agent Learning Flywheel (Augment Code 2026) — all confirm cross-cycle learning improves agent performance | Execution Critique saves lessons to `.cali-product-workflow/lessons-learned/{date}-{name}.md`. Setup stage (0b) automatically reads past lessons and injects them into the session with explicit instruction to check for repeated patterns. No user action needed. The gap registry tracks recurring patterns. Academic research (CODESKILL 2026, FORGE 2026) confirms that injecting past learnings as structured context improves future agent decisions, especially for meta-knowledge (validation routines, failure patterns) rather than task-specific code. | **Captured and injected, with academic support, but limitations remain.** The approach is validated by research (CODESKILL: distilled experiences improve agent performance without parameter updates; FORGE: population-based memory evolution works across sessions). However: (1) the same model that made mistakes evaluates the lessons, (2) context rot can cause mid-session forgetting, (3) lesson quality depends on Execution Critique's blind spots, (4) the workflow cannot auto-verify whether the LLM actually followed the lessons. |
| 10 | **Code complexity growth** — AI-generated code tends to increase complexity over time. Static analysis warnings +30%, code complexity +41% after month 2 of sustained AI use. | Cursor longitudinal study; Faros AI 2025 (AI-assisted teams produce 47% more PRs/day but activity ≠ productivity) | Execution Critique includes anti-pattern detection (god functions >100 lines, global mutable state). Execution's optional Code Quality Gate (`codequality-review` skill or manual eslint/tsc fallback). | **Caught too late, static analysis is optional.** Complexity analysis happens after code is written. The Code Quality Gate with static analysis (eslint, tsc) lives in execution.md as an optional step — it's NOT in Verification, so it's easy to skip. The workflow has no mechanism to prevent complexity during generation — only flag it after. Preventive measures (architecture boundaries, human review) are outside workflow scope. |
