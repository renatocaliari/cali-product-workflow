...
| 9 | **Pipeline memory loss** — the pipeline has no cross-session memory of its own failure patterns. The same mistake repeats in every cycle. | Flamehaven 2026 (AI pipeline analysis) | Execution Critique now saves lessons to `.cali-product-workflow/lessons-learned/{date}-{name}.md`. Setup stage (0b) automatically reads past lessons and injects them into the session with an instruction to check for repeated patterns. No user action needed. The gap registry tracks recurring patterns. | **Captured AND injected, but the LLM may still ignore them.** The lessons are now auto-read and prefixed to every workflow session. However, the same model that made the original mistakes is now evaluating the lessons. Context rot can also cause the LLM to forget the lessons mid-session. The quality of lessons (how specific, how actionable) depends entirely on the Execution Critique generation, which suffers from the same blind spots as the rest of the pipeline. |
